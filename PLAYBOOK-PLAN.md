...
### Changelog
- **04.09.2026 (Nachtrag)**: DREI ÜBERSEHENE STELLEN mit Team-Zweiteilung — von Steffen entdeckt, weil er in der Aufstellung weiter zwischen 1. und 2. Mannschaft wählen konnte. (1) `aufstellung.html` hatte den Umschalter „1. XV — 2. Bundesliga / 2. XV — Regionalliga" samt CSS, Handlern und getrennter Ablage `lineup[activeTeam]`; zusätzlich waren die Spieltage als `games1`/`games2` **hart im Code** hinterlegt, beide aus der Saison 2025/26 — deshalb stand dort „Keine weiteren Spiele geplant", obwohl am 06.09. gespielt wird. Umschalter raus, Spielplan kommt jetzt aus `data/liga.json` (nur `ourGame`, Gegner über `shortTeam()` gekürzt); `loadGames()` läuft `await`-vor dem Aufbau der Oberfläche, weil Spieltag-Auswahl und Match-Kopfzeile die Daten brauchen. **Datenerhalt**: Der Firebase-Schlüssel `'1'` bleibt als `TEAM_KEY` bestehen, statt die Ablage flach zu ziehen — so blieben die veröffentlichte Aufstellung und Steffens laufender Entwurf unangetastet; die verwaisten Daten unter `'2'` bleiben liegen und werden nicht mehr gelesen. (2) `mannschaftsrat.html` hatte im Ziele-Bereich Tabs „Alle / 1. XV / 2. XV" mit Zielen wie „2. Bundesliga Nord — Klassenerhalt sichern"; Tabs raus, eine gemeinsame Liste, Überschrift „Saisonziele Rückrunde" → „Saisonziele 2026/27", faktisch überholte Ziele entfernt, die übrigen inhaltlich unverändert übernommen (neue Ziele gehören ins Team, nicht in einen Commit). (3) `index.html` Maßnahme 3 nannte Jonas Hinz als Verantwortlichen — Name entfernt. ⚠️ **Lehre**: Die erste Vollständigkeitsprüfung lief als `grep -rn … | head -20` — die Kappung hat genau die Zeile mit dem Umschalter geschluckt. **Bei Vollständigkeitsprüfungen nie mit `head` kappen**, lieber pro Datei zählen (`for f in *.html; do echo "$f: $(grep -c muster "$f")"; done`).
...
- ✅ **`lineup_history` freigegeben (04.09.2026)** — Aufstellungs-Historie funktioniert wieder. Ursache: Die Realtime-Database-Rules listen jeden erlaubten Pfad einzeln mit `".read"/".write": "auth != null"` auf und fangen alles Übrige mit `"$other": { ".read": false, ".write": false }` ab; `lineup_history` fehlte in der Liste und fiel deshalb auf `$other` — deshalb `permission_denied` auf `aufstellung.html`, seit das Feature gebaut wurde. Fix: ein Block nach `lineup_meta`, gleiches Muster wie die anderen elf Pfade, in der Firebase Console veröffentlicht. Verifiziert: `aufstellung.html` live ohne Console-Fehler, PUT/GET/DELETE auf einem Testeintrag alle HTTP 200, Testeintrag wieder entfernt. ⚠️ **Merke für neue Firebase-Pfade**: Wer im Code einen neuen `db.ref('…')`-Pfad einführt, MUSS ihn in den Rules ergänzen — sonst schlägt er still fehl. Gegenprobe: alle Pfade im Code sammeln (`grep -rho "ref('[a-zA-Z_/]*'"`) und gegen die Rules abgleichen. Kein firebase-CLI im Projekt, Rules sind per REST mit Anonymous-Auth nicht schreibbar → Änderung nur über die Console.
...
- ✅ **DSGVO — `players_meta` bereinigt (04.09.2026)**: Die Geburtsdaten der 15 ausgeschiedenen Spieler sind auf Steffens Freigabe hin aus Firebase gelöscht (Zweckfortfall — sie stehen in keiner Kaderliste mehr und wurden nirgends angezeigt). Danach 41 Einträge, exakt die aktiven Spieler mit hinterlegtem Geburtsdatum; Moritz Janosch, Simon Reichelt und Lukas Rathke haben keins. ⚠️ **Zähl-Falle**: `players_meta` kommt als ARRAY mit `null`-Lücken zurück (höchste ID 71 → Länge 72). `Object.keys()` auf der vollen Antwort zählt die Löcher mit; für echte Einträge immer `?shallow=true` abfragen.
